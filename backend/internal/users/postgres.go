...
func (r *repo) GetUserByID(id string, ctx context.Context) (*User, error) {
	...
	result := r.pool.QueryRow(ctx, query, id)
	user := User{}
	if err := result.Scan(
		&user.Id,
		&user.Username,
...
func (r *repo) GetUserByUsername(username string, ctx context.Context) (*User, error) {
	logger := logging.GetLogger()
	logger.Debugf("Getting user by username %s", username)
	...
	result := r.pool.QueryRow(ctx, query, username)
	user := User{}
	err := result.Scan(
		&user.Id,
		&user.Username,
...
func (r *repo) InsertUser(user *User, ctx context.Context) (*User, error) {
	...
	query := `INSERT INTO users (
					id, username, pfp_url, active, hashed_password, created_at, updated_at
				) VALUES ($1, $2, $3, $4, $5, $6, $7)
				RETURNING id, username, pfp_url, created_at, updated_at, active, hashed_password`

	result := r.pool.QueryRow(ctx, query,
		user.Id, user.Username, user.PfpURL, user.Active, user.HashedPassword, user.CreatedAt, user.UpdatedAt,
	)
...
func (r *repo) SetActive(id string, active bool, ctx context.Context) (bool, error) {
	logger := logging.GetLogger()
	logger.Debugf("Setting active for user %s to %t", id, active)
	...
	result := r.pool.QueryRow(ctx, query, active, id)
...
func (r *repo) UpdatePassword(id string, hashedPassword string, ctx context.Context) (string, error) {
	logger := logging.GetLogger()
	logger.Debugf("Updating password for user %s", id)
	...
	result := r.pool.QueryRow(ctx, query, hashedPassword, id)
